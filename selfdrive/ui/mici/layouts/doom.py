...
class DoomLayout(NavWidget):
  ...
  def _draw_view(self):
    ...
    stripe_w = max(4, int(6 * self._ui_scale))
  ...
  def _draw_crosshair(self):
    cx = int(self._view_rect.x + self._view_rect.width / 2)
    cy = int(self._view_rect.y + self._view_rect.height / 2)
    color = rl.Color(255, 230, 210, 255)
    outer = int(18 * self._ui_scale)
    inner = int(6 * self._ui_scale)
    rl.draw_line(cx - outer, cy, cx - inner, cy, color)
    rl.draw_line(cx + inner, cy, cx + outer, cy, color)
    rl.draw_line(cx, cy - outer, cx, cy - inner, color)
    rl.draw_line(cx, cy + inner, cx, cy + outer, color)

  def _draw_hud(self):
    rl.draw_rectangle_rounded(self._hud_rect, 0.22, 12, rl.Color(15, 0, 0, 190))
    title_font = int(34 * self._ui_scale)
  ...
  def _draw_virtual_pad(self):
    for slot in range(len(self._touch_origin)):
      ...
      rl.draw_circle(int(origin.x), int(origin.y), radius, rl.Color(255, 255, 255, 45))
      rl.draw_circle_lines(int(origin.x), int(origin.y), radius, rl.Color(255, 255, 255, 90))
      rl.draw_circle(int(origin.x + dx), int(origin.y + dy), knob_radius, rl.Color(255, 80, 80, 180))
  ...
  def _draw_overlays(self):
    ...
    if self._message_time > 0.0:
      font_size = int(68 * self._ui_scale)
    ...
    if self._win or self._dead:
      msg = "TAP TO PLAY AGAIN"
      hint_font = int(34 * self._ui_scale)
